...
def encode_class(orders):
    # classes is an array with length powers
    # joins the orders made by a power with ^
    # joins all orders made by all powers with &
...
# entry_to_vectors returns 3 vectors and 1 string:
# [0] attributes: list of np array of attributes/features
# [1] classes: list of orders
# [2] season_phase: shortform of season
...
def entry_to_vectors(phase):
    ...
    attributes = list()
    classes = list()
    keys = list() # Each key represents a dataset for a separate model
    ...
    # For each order, append attribute to attributes, 
    # the given order to that unit to classes
    # and the unit to units
